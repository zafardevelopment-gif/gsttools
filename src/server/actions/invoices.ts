...
/**
 * Create an invoice from the UI.
 *
 * All the actual work lives in `@/server/services/invoices` so that the
 * WhatsApp voice-bill engine, recurring/cron generation and the automation API
 * produce byte-identical invoices (same numbering RPC, same GST maths, same
 * stock postings). This wrapper only supplies the request-scoped bits: the
 * session's tenant/user, the RLS client, plan gating, and cache revalidation.
 */
export async function createInvoiceAction(
  ...
  const res = await createInvoice({
    ...
    input,
    source: "ui",
    checkPlanLimit: canCreateInvoice,
  });
  if (res.error || !res.id) {
    return { error: res.error ?? "Could not create invoice." };
  }
